...
import { useRoverControls } from '@/composables/useRoverControls.js'
...
onMounted(async () => {
  ...
  try {
    const gltf = await new Promise((resolve, reject) => {
      terrainLoader.load(url, resolve, undefined, reject)
    })
    ...
    // Init rover controls
    roverControls = useRoverControls(camera, canvas)
    roverControls.setTerrainMesh(terrainGroup)
    roverControls.init()

    // Load rover model and place on terrain
    await roverControls.loadRover(scene)
    roverControls.placeOnTerrain()
